"""Tests for the compile-safe NPU fused-op boundary."""
...
def test_fake_preserves_output_metadata():
    with FakeTensorMode():
        qkv = torch.empty((2, 4096), dtype=torch.bfloat16)
        sin = torch.empty((2, 128), dtype=torch.bfloat16)
        cos = torch.empty((2, 128), dtype=torch.bfloat16)
...
def test_compiled_op_matches_external_kernel(batch_size):
    ...
    eager = call_op(qkv, sin, cos, weight)
    ...
    for actual_tensor, expected_tensor in zip(eager, expected):
